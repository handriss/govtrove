...
import (
	...
	"github.com/handriss/govtrove/jobs/internal/ingest"
...
func handler(ctx context.Context, event json.RawMessage) (*Output, error) {
	...
	runID, err := db.CreateIngestionRun(ctx, "ingest-archived")
	if err != nil {
		return nil, fmt.Errorf("create ingestion run: %w", err)
	}

	upserted, recordCount, err := runPipeline(ctx, runID, snapshotDate, input.File.S3Key)
...
func runPipeline(ctx context.Context, runID uuid.UUID, snapshotDate time.Time, s3Key string) (upserted, recordCount int, err error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("get S3 object: %w", err)
	}
	...
	if err != nil {
		return 0, 0, fmt.Errorf("create download entry: %w", err)
	}
	...
	if _, err := db.BulkInsertSnapCSV(ctx, runID, snapshotDate, downloadID, snapRows); err != nil {
		...
		return 0, recordCount, fmt.Errorf("bulk insert: %w", err)
	}

	db.CompleteCSVDownloadEntry(ctx, downloadID, recordCount, 0)

	opps := make([]reconcile.Opportunity, 0, len(result.Rows))
	for _, raw := range result.Rows {
	...
	if err != nil {
		return 0, recordCount, fmt.Errorf("upsert opportunities: %w", err)
	}

	return inserted, recordCount, nil
}
