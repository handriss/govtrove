...
func handler(ctx context.Context, event json.RawMessage) (*Output, error) {
	...
	runID, err := db.CreateIngestionRun(ctx, "snapshot-csv")
	if err != nil {
...
func runPipeline(ctx context.Context, runID uuid.UUID, snapshotDate time.Time, s3Key string) (*pipelineStats, error) {
	...
	prevRunID, _, prevErr := db.GetLastCompletedRun(ctx, "snapshot-csv")
	if prevErr != nil {
	...
	stats.oppsUpserted = inserted

	if prevRunID != uuid.Nil {
		deactivated, err := db.MarkDisappearedInactive(ctx, runID)
		if err != nil {
			logger.Error("failed to mark disappeared inactive", "error", err)
		} else if deactivated > 0 {
			logger.Info("marked disappeared opportunities inactive", "count", deactivated)
		}
	}
