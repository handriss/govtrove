...
resource "aws_cloudfront_response_headers_policy" "landing" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      content_security_policy = "default-src 'self'; script-src 'self' https://unpkg.com https://cdn.counter.dev 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://api.govtrove.com https://t.counter.dev https://unpkg.com; img-src 'self' data:; frame-ancestors 'none'; base-uri 'self'; form-action 'self' https://api.govtrove.com"
      override               = true
    }
  }
}
