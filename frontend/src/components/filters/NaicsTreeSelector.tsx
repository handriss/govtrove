...
interface NaicsTreeSelectorProps {
  selected: string[];
...
export default function NaicsTreeSelector({
  selected,
  onChange,
  facets,
  label = 'NAICS',
  ...
  const searchResults = useMemo(() => {
    ...
    return searchNaicsCodes(search);
  }, [search]);
  ...
  const handleSearchSelect = useCallback(
    (code: string) => {
      ...
      // Auto-expand ancestors so the code is visible in tree view
  ...
  function TreeNodeRow({ node }: { node: NaicsTreeNode }) {
  ...
  // Panel content (shared between dropdown and inline)
  const panelContent = (
    <>
      {/* Search input */}
